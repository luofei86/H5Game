...
COLUMNS = " user_id, active_id, question_ids, play_question_id, result "
INSERT_SQL = '''INSERT INTO ''' + TABLE_NAME + '''(id, user_id, active_id, ''' \
		...
		+ ''' VALUES (null, %s, %s, %s, %s, 0, 0, now(), now())'''
UPDATE_SQL = ''' UPDATE ''' + TABLE_NAME + ''' SET play_question_id = %s WHERE user_id = %s AND active_id = %s'''
UPDATE_RESULT_SQL = '''UPDATE ''' + TABLE_NAME + ''' SET result = %s WHERE user_id = %s AND active_id = %s'''
...
class UserPlayOriginGameInfoDao:
	...
	def updateResult(self, userId, activeId, result):
		dbConn = get_db()
		with closing(dbConn.cursor()) as cur:
			cur.execute(UPDATE_RESULT_SQL, (str(result), str(userId), str(activeId)))
		dbConn.commit()

	def updatePlayQuestionId(self, userId, activeId, playQuestionId):
		dbConn = get_db()
		with closing(dbConn.cursor()) as cur:
			cur.execute(UPDATE_RESULT_SQL, (str(playQuestionId), str(userId), str(activeId)))
		dbConn.commit()
...
#######id, keyword, sign_word, url, title content, resource_url
	def _toObject(self, db_item):
		if db_item is None:
			return None
		return UserPlayOriginGameInfo(db_item[0], db_item[1], db_item[2], db_item[3], db_item[4])
